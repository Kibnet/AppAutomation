...
public static class AutomationElementWaitExtensions
{
    ...
    private static int ReadListBoxItemCount(ListBox listBox)
    {
        ArgumentNullException.ThrowIfNull(listBox);

        try
        {
            var direct = listBox.Items.Length;
            if (direct > 0)
            {
                return direct;
            }
        ...
        var descendants = listBox
            .FindAllDescendants()
            .Where(node => node != listBox)
            .ToArray();

        var listItemCount = descendants.Count(node => node.ControlType == ControlType.ListItem);
        if (listItemCount > 0)
        {
            return listItemCount;
        }

        var textLikeCount = descendants.Count(node => node.ControlType == ControlType.Text && !string.IsNullOrWhiteSpace(node.Name));
        if (textLikeCount > 0)
        {
    ...
    private static string[] ReadListBoxItemsText(ListBox listBox)
    {
        ArgumentNullException.ThrowIfNull(listBox);
    ...
    public static bool WaitUntilItemSelected(this Tree tree, TreeItem targetItem, int timeoutMs = 5000)
    {
        var waitResult = UiWait.TryUntil(
            ...
            actual => ReferenceEquals(actual, targetItem)
                || (actual is not null && actual.AutomationId == targetItem.AutomationId && actual.Name == targetItem.Name),
            new UiWaitOptions { Timeout = TimeSpan.FromMilliseconds(timeoutMs) });
    ...
    private static string ReadText(AutomationElement element)
    {
        ...
        if (nestedTextBox is not null && !string.IsNullOrWhiteSpace(nestedTextBox.Text))
        {
