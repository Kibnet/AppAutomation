...
| EXEC | Publish verification follow-up | 0.93 | Нужно запустить build tests | Убрать automatic post-publish verification и открыть PR | Нет | Да: пользователь подтвердил, что успешного NuGet push достаточно | Release workflow больше не валится на задержке индексации NuGet после успешной публикации; published consumer verification остаётся ручным диагностическим скриптом | `eng/publish-nuget.ps1`, `docs/appautomation/publishing.md`, `tests/AppAutomation.Build.Tests/PublishNugetScriptTests.cs` |
| EXEC | Publish verification validation | 0.94 | Нет | Закоммитить, запушить и открыть PR | Нет | Нет | Targeted build tests, full build и full test run прошли; остались только существующие warnings NU1903/CA* | `dotnet test --project tests/AppAutomation.Build.Tests/AppAutomation.Build.Tests.csproj -c Release`, `dotnet build AppAutomation.sln -c Release --no-restore`, `dotnet test --solution AppAutomation.sln -c Release --no-build` |
