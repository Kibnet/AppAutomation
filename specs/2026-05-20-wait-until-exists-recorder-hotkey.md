...
| EXEC | FlaUI recorded smoke fix | 0.9 | Full solution retry не запускался после узких FlaUI проверок | Build, commit and push PR update | Нет | Да, пользователь сообщил, что `Recorded_RecordedSmoke_20260520_170259` проходит headless и падает во FlaUI | Найден setup failure до тела теста: placement строго ждал exact outer bounds; для `Size=null` теперь сохраняется текущий размер через `SWP_NOSIZE`, а explicit size проверяется с отдельным desktop-size tolerance | `src/AppAutomation.FlaUI/Session/DesktopWindowPlacementService.cs`, `sample/DotnetDebug.AppAutomation.FlaUI.Tests/Tests/DesktopWindowPlacementTests.cs`, `specs/2026-05-20-wait-until-exists-recorder-hotkey.md` |
| EXEC | WaitUntilExists contrast tests | 0.95 | Нет | Запустить targeted abstractions tests | Нет | Да, пользователь попросил два теста, отличающихся наличием `WaitUntilExists` | Добавлена наглядная regression-пара: сценарий с `WaitUntilExists` проходит, сценарий без этой строки ожидаемо бросает not-found exception | `tests/AppAutomation.Abstractions.Tests/UiPageExtensionsTests.cs`, `specs/2026-05-20-wait-until-exists-recorder-hotkey.md` |
| EXEC | Real app delayed element contrast | 0.9 | Нужно проверить Headless и FlaUI targeted tests | Запустить runtime-targeted проверки | Нет | Да, пользователь уточнил, что contrast должен быть в реальном приложении | Unit-level contrast заменён реальным app flow: delayed label создаётся в UI после задержки; общий inherited scenario содержит два теста, где `WaitUntilTextEquals` сразу следует после `WaitUntilExists`, а вариант без exists wait падает на not-found resolve | `sample/DotnetDebug.Avalonia/MainWindow.axaml`, `sample/DotnetDebug.Avalonia/MainWindow.axaml.cs`, `sample/DotnetDebug.AppAutomation.Authoring/Pages/MainWindowPage.DelayedStatus.cs`, `sample/DotnetDebug.AppAutomation.Authoring/Tests/MainWindowScenariosBase.cs`, `tests/AppAutomation.Abstractions.Tests/UiPageExtensionsTests.cs`, `specs/2026-05-20-wait-until-exists-recorder-hotkey.md` |
